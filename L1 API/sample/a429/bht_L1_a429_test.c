                ...
                do
                {
                    ...
                    if(option1 != DIAG_EXIT_MENU)
                    {
                        if(1 == option1)
                        {
                            ...
                            printf("day  : %d\nhour : %d\nmin  : %d\nsec  : %d\nms   : %d\nus   : %d\n",
                                irigb_time.tm_day, irigb_time.tm_hour, irigb_time.tm_min, 
                                irigb_time.tm_sec, irigb_time.tm_ms, irigb_time.tm_us);
                        }
                        else if(2 == option1)
                        {
                            irigb_time.tm_day = DIAG_GetNumber("day", INPUT_DATA_FORMAT_DEC, 0, 365);
                            irigb_time.tm_hour = DIAG_GetNumber("hour", INPUT_DATA_FORMAT_DEC, 0, 23);
                            irigb_time.tm_min = DIAG_GetNumber("min", INPUT_DATA_FORMAT_DEC, 0, 59);
                            irigb_time.tm_sec = DIAG_GetNumber("sec", INPUT_DATA_FORMAT_DEC, 0, 59);
                            irigb_time.tm_ms = DIAG_GetNumber("ms", INPUT_DATA_FORMAT_DEC, 0, 999);
                            irigb_time.tm_us = DIAG_GetNumber("us", INPUT_DATA_FORMAT_DEC, 0, 999);
            ...
            do
            {
                ...
                case 1:
                    ...
                    do
                    {
                        ...
                        printf("4.baud rate input\n");
                        printf("%d. EXIT\n", DIAG_EXIT_MENU);

                        if (DIAG_INPUT_FAIL == DIAG_GetMenuOption(&option2, 4))
                            continue;
